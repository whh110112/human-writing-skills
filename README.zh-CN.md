...
时才可使用；`texture` 负责叙事距离、比喻与感官负载、单行段落成串、动作后重复
解释情绪以及人物资料倾倒。
...
每个阶段应放到新的模型会话或独立 API 请求运行。自动模式会保留逻辑、AI 痕迹和校对，再按本章出现的人物、关系、空间、精确数字、持续对白和文风密度线索追加专项阶段。只有同时提供前章或账本时才会追加 `serial`；清单会说明选择和跳过原因。
